...
def test_route_max_skills():
    ...
    res = SkillRouter.route(prompt, max_skills=2)
    assert len(res) == 2

def test_load_skill_exists():
    """load_skill('snowflake-security') returns non-empty string."""
    res = SkillRouter.load_skill("snowflake-security")
    assert res is not None
    assert len(res) > 0

def test_load_skill_missing():
    """load_skill('nonexistent') returns None."""
    res = SkillRouter.load_skill("nonexistent_skill_xyz")
    assert res is None

def test_load_critical_rules():
    """load_critical_rules extracts a section, not full content."""
    res = SkillRouter.load_critical_rules("snowflake-security")
    assert res is not None
    assert "Never grant privileges directly to users" in res
    assert "# Snowflake Security" not in res # It should only extract the section

def test_skill_index_lists_all():
    """skill_index() mentions all 12 skill names."""
    idx = SkillRouter.skill_index()
    assert len(SKILL_MAP) == 12
...
def test_skill_map_files_exist():
    """Every key in SKILL_MAP has a corresponding .md file."""
